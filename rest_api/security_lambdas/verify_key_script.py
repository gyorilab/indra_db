...
def lambda_handler(event, context):
    ...
    awsAccountId = tmp[4]
    print('a', tmp, apiGatewayArnTmp, awsAccountId)
    ...
    # principal these are made available by APIGW like so: $context.authorizer.
    # <key> additional context is cached
    context = {
...
class AuthPolicy(object):
    ...
    def _addMethod(self, effect, verb, resource, conditions):
        ...
        if verb != '*' and not hasattr(HttpVerb, verb):
            raise NameError('Invalid HTTP verb %s. Allowed verbs in HttpVerb '
                            'class' % verb)

        resourcePattern = re.compile(self.pathRegex)
        if not resourcePattern.match(resource):
            raise NameError('Invalid resource path: %s. Path should match %s.'
                            % (resource, self.pathRegex))

        if resource[:1] == '/':
            resource = resource[1:]

        resourceArn = 'arn:aws:execute-api:%s:%s:%s/%s/%s/%s' \
                      % (self.region, self.awsAccountId, self.restApiId,
    ...
    def _getEmptyStatement(self, effect):
        '''Get an empty statement prepopulated with an action and effect.'''
        statement = {
    ...
    def _getStatementForEffect(self, effect, methods):
        ...
        if len(methods) > 0:
            ...
            for curMethod in methods:
                arn = curMethod['resourceArn']
                if curMethod['conditions'] is None \
                   or len(curMethod['conditions']) == 0:
                    statement['Resource'].append(arn)
                else:
                    conditionalStatement = self._getEmptyStatement(effect)
                    conditionalStatement['Resource'].append(arn)
    ...
    def allowAllMethods(self):
        '''Add * to the policy to authorize access to all methods of an API.'''
        self._addMethod('Allow', HttpVerb.ALL, '*', [])

    def denyAllMethods(self):
        '''Add * to the policy to deny access to all methods of an API.'''
        self._addMethod('Deny', HttpVerb.ALL, '*', [])

    def allowMethod(self, verb, resource):
        '''Adds an API Gateway method to the list of allowed methods.'''
        self._addMethod('Allow', verb, resource, [])

    def denyMethod(self, verb, resource):
        '''Adds an API Gateway method to the list of denied methods.'''
        self._addMethod('Deny', verb, resource, [])

    def allowMethodWithConditions(self, verb, resource, conditions):
        '''Adds an API Gateway method to the list of allowed methods.

        Also includes a condition for the policy statement. More on AWS policy
        conditions here:
    ...
    def build(self):
        '''Generate the policy document from the internal lists of conditions.

        This will generate a policy with two main statements for the effect:
        one statement for Allow and one statement for Deny. Methods that
        includes conditions will have their own statement in the policy.
        '''
